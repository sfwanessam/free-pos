...
const form = useForm({
    ...
    image: null,
    is_a_service: props.product.is_a_service,
...
const submit = () => {
    form.post(route('catalog.products.update', props.product.id));
...
<template>
    ...
    <Head :title="$t('products.edit_product')" />
                ...
                <div class="mb-4">
                    <nav class="flex mb-5" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                            ...
                            <POSBreadcrumbItem :label="$t('products.edit_product')" />
                        </ol>
                    </nav>
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">{{
                        $t('products.edit_product') }}</h1>
                </div>
