...
<template>
    ...
    <Head :title="$t('categories.new_category')" />
                ...
                <div class="mb-4">
                    <nav class="flex mb-5" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                            ...
                            <POSBreadcrumbItem :url="route('catalog.categories.index')" :label="$t('categories.title')"/>
                            <POSBreadcrumbItem :label="$t('categories.new_category')"/>
                        </ol>
                    </nav>
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">{{
                        $t('categories.new_category') }}</h1>
                </div>
                    ...
                    <form @submit.prevent="submit">
                        <div class="space-y-4">
                            <div class="pt-1">
                                ...
                                <POSTextInput v-model="form.name" name="name" :placeholder="$t('categories.category_name_placeholder')"/>
                            ...
                            <div class="pt-1">
                                ...
                                <div class="mt-1 text-sm text-gray-500 dark:text-gray-300">{{$t('categories.image_hint') }}</div>
                            ...
                            <div class="pt-1">
                                ...
                                <POSSelectInput v-model="form.active">
                                    <option value="1">{{$t('categories.active')}}</option>
                                    <option value="0">{{$t('categories.disactive')}}</option>
                                </POSSelectInput>
                            </div>
                            ...
                            <div class="pt-1">
                                ...
                                <Link :href="route('catalog.categories.index')" class="inline-flex ms-2 items-center px-3 py-2 text-sm font-medium text-center text-gray rounded-lg bg-white border border-gray-300 focus:ring-4 ">
